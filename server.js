...
if (!SPOTIFY_CLIENT_ID || !APP_BASE_URL) {
  console.error(
    "Missing required env vars: SPOTIFY_CLIENT_ID and APP_BASE_URL",
  );
  process.exit(1);
}
...
try {
  ...
  console.error(
    "APP_BASE_URL must be an absolute URL, e.g. http://localhost:3000",
  );
  process.exit(1);
}
...
function rateLimitGeneratePlaylist(req, res, next) {
  ...
  if (bucket.count >= RATE_LIMIT_MAX_REQUESTS) {
    ...
    return res.status(429).json({
      ...
      user_message:
        "Too many playlist generation attempts. Please wait a minute and try again.",
    });
  }
...
async function spotifyApiRequest(
  ...
  if (
    ...
    retryCount < MAX_SPOTIFY_RETRIES
  ) {
  ...
  if (resp.status === 429) {
    throw new HttpError(
      429,
      `Spotify rate limit: ${spotifyMessage}`,
  ...
  if (resp.status >= 500) {
    throw new HttpError(
      502,
      `Spotify upstream error: ${spotifyMessage}`,
      "Spotify is temporarily unavailable. Please retry.",
    );
  }

  throw new HttpError(
    resp.status,
    `Spotify API ${endpoint} failed: ${spotifyMessage}`,
    spotifyMessage,
  );
}
...
function normalizeGeneratePayload(body) {
  ...
  const distanceKm =
    distanceKmRaw == null || distanceKmRaw === "" ? NaN : Number(distanceKmRaw);
  if (
  ...
  if (!Number.isNaN(distanceKm) && distanceKm > 0 && !pace) {
    throw new HttpError(
      400,
      "Missing pace for distance",
      "Pace is required when distance is provided.",
    );
  }
...
app.get("/api/session", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ connected: false, error: "Unable to verify session." });
  }
});
...
app.get("/api/top-tracks", async (req, res) => {
  try {
    ...
    const limit = Number.isFinite(limitNum)
      ? Math.max(1, Math.min(50, Math.trunc(limitNum)))
      : 20;
    ...
    const userMessage =
      err instanceof HttpError ? err.userMessage : "Failed to load top tracks.";
    const errorMessage = err instanceof Error ? err.message : "Unknown error";
    console.error(`top-tracks error (${status}): ${errorMessage}`);
    return res
      .status(status)
      .json({ error: errorMessage, user_message: userMessage });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (!accessToken) {
        return res.status(401).json({
          error: "Not authenticated with Spotify.",
          user_message: "Connect Spotify before generating a playlist.",
        });
      }
      ...
      if (normalized.pace) {
        try {
          ...
          throw new HttpError(
            400,
            err instanceof Error ? err.message : "Invalid pace",
            "Invalid pace. Use formats like 5:30 or 5.5 min/km.",
          );
        }
      }
      ...
      const { selectedUris, totalDuration } = pickTracksForDuration(
        tracks,
        targetDurationMs,
      );
      ...
      if (selectedUris.length === 0) {
        return res.status(404).json({
          ...
          user_message:
            "No favorite tracks matched your explicit-content setting. Try allowing explicit tracks.",
        });
      }
      ...
      const playlist = await spotifyApiRequest(accessToken, `/me/playlists`, {
        method: "POST",
        body: JSON.stringify({
          name: playlistName,
